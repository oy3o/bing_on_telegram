...
impl CoreBPE {
    ...
    fn _count(&self, text: &str, allowed_special: &HashSet<&str>) -> usize {
        self._encode_native(text, &allowed_special).0.len()
    }
}
...
impl CoreBPE {
    ...
    fn count(&self, py: Python, text: &str, allowed_special: HashSet<&str>) -> PyResult<usize> {
        Ok(self._count(text, &allowed_special))
    }
}

#[pymodule]
fn _tokcos(_py: Python, m: &PyModule) -> PyResult<()> {
    m.add_class::<CoreBPE>()?;
    Ok(())
}
